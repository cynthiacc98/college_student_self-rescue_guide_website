...
- [x] 资源列表/详情页：未登录拦截 + 详情页服务端保护
- [x] 搜索页：关键字+分类+排序，分页
- [x] 分类页：列表筛选与分类详情分页
- [x] 开发期种子/辅助接口：`/api/dev/seed`、`/api/dev/resources`、`/api/dev/users`
- [x] 端到端验证：注册首个管理员→登录→访问资源详情→访问后台

## 里程碑 M3：管理员后台（2-3 天）
- [x] 后台首页仪表盘（计数、趋势占位）
- [x] 分类管理：列表、创建、编辑、启用/禁用、排序、删除
- [x] 资料管理：列表、创建、编辑、删除（封面 URL、简介、标签、分类、公开）
- [x] 基础表单校验（zod + RHF）
